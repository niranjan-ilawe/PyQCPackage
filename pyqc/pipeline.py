...
from pyqc.kit_qc.df_creation_scripts import get_qc123_data, get_qc167_data, get_qc149_data, get_historical_data
...
def run_qc167_pipeline(days=3):
    ...
    print("---- Getting QC167 Data ----")
    try:
        df = get_qc167_data(days)
        print(colored("---- Uploading QC167 Data ----", "green"))
        cur = conn.cursor()
        cur.execute(f"DELETE FROM kitqc.metrics_stg;")
        conn.commit()
        batch_upload_df(
...
def run_histoical_pipeline(days=100):
    ...
    try:
        ...
        print(colored("---- Uploading Historical Data ----", "green"))
        cur = conn.cursor()
        cur.execute(f"DELETE FROM kitqc.metrics_stg;")
        conn.commit()
        batch_upload_df(
        ...
        print(colored("---- Skipping Historical Data ----", "yellow"))
